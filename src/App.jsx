...
function App() {
  ...
  const getSlider = async (id = 1) => {
    try {
      setLoading(true);
      const Data = await axios.get(`${HomeApi}/meta/anilist/trending?page=1`);
  ...
  const getTopAiring = async (id = 1) => {
    try {
      setLoading(true);
      const Data = await axios.get(
        `${HomeApi}/meta/anilist/trending?page=${id}&perPage=20`
      );
  ...
  const handelChanges = async (val) => {
    try {
      setLoading(false);
      const searchRes = await axios.get(`${HomeApi}/meta/anilist/${val}`);
      if (val === "") {
  ...
  return (
    <Router className="App">
      ...
      <Routes>
        ...
        />
        <Route exact path="/watch/:episodeId/:animeId" element={<Stream />} />
